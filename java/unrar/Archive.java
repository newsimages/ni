...
import unrar.exception.RarException.RarExceptionType;
import unrar.io.IReadOnlyAccess;
import unrar.io.ReadOnlyAccessByteArray;
import unrar.io.ReadOnlyAccessFile;
import unrar.rarfile.AVHeader;
import unrar.rarfile.BaseBlock;
import unrar.rarfile.BlockHeader;
import unrar.rarfile.CommentHeader;
import unrar.rarfile.EAHeader;
import unrar.rarfile.EndArcHeader;
import unrar.rarfile.FileHeader;
import unrar.rarfile.MacInfoHeader;
import unrar.rarfile.MainHeader;
import unrar.rarfile.MarkHeader;
import unrar.rarfile.ProtectHeader;
import unrar.rarfile.SignHeader;
import unrar.rarfile.SubBlockHeader;
import unrar.rarfile.UnixOwnersHeader;
import unrar.rarfile.UnrarHeadertype;
import unrar.unpack.ComprDataIO;
...
public class Archive implements Closeable {
	...
	public Archive(File file, UnrarCallback unrarCallback, String password, boolean test)
		...
		this(new ReadOnlyAccessFile(file, password), file.getAbsolutePath(), unrarCallback, password, test);
	}

	public Archive(byte[] data, String path, boolean test) throws RarException, IOException {
		this(data, path, null, test);
	}

	/**
	 * create a new archive object using the given in-memory data
	 * 
	 * @param data
	 *            the data to extract
	 * @throws RarException
	 */
	public Archive(byte[] data, String path, UnrarCallback unrarCallback, boolean test)
			throws RarException, IOException {
		this(new ReadOnlyAccessByteArray(data), path, unrarCallback, null, test);
	}

	private Archive(IReadOnlyAccess rof, String path, UnrarCallback unrarCallback, String password, boolean test)
			throws RarException, IOException {
	...
	public IReadOnlyAccess openRof(String newPath)
			throws IOException {
		if(rof instanceof ReadOnlyAccessFile){
			...
			String m = "Don't know how to open data stream from name " + newPath + " (you must override Archive#openRof)";
			System.err.println(m);
			throw new IOException(m);
		}
	}
	
	void setRof(IReadOnlyAccess rof) throws IOException {
		totalPackedSize = 0L;
		totalPackedRead = 0L;
		close();
		this.rof = rof;
